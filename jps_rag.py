...
def sanitize_filename(path: Path) -> Path:
    ...
    return path.with_name(name + ".docx")
...
def convert_doc_to_docx(doc_path: Path) -> Path:
    ...
    safe_name = re.sub(r'[<>:"/\\|?*]', "_", doc_path.stem)
    docx_path = doc_path.with_name(safe_name + ".docx")
    ...
    try:
        ...
        doc.SaveAs(abs_docx_path, FileFormat=16)  # 16 = wdFormatDocumentDefault (.docx)
        doc.Close()
    ...
    if abs_docx_path:
        print(f"✅ Converted {doc_path} -> {abs_docx_path}")
        return Path(abs_docx_path)
    else:
        return None
...
def ingest_path(path: Path, doc_type="general"):
    ...
    for file in files:
        ...
        if archive_dir in file.parents:
            continue
        ...
        try:
            ...
            if file.suffix.lower() == ".doc":
                ...
                if new_file is None:
                    print(f"Skipping {file} (failed conversion)")
                    continue
                file.unlink()  # delete original .doc
                file = new_file
                text = load_docx(file)
            ...
            metadatas = [
                {"source": str(file), "type": doc_type, "section": f"chunk_{i}", "chunk_index": i}
